...
public class OrderController {
    ...
    public OrderController(ShoppingCartService shoppingCartService, OrderService orderService,
                           UserService userService, ResponseDtoMapper<OrderResponseDto,
    ...
    public List<OrderResponseDto> getOrderHistory(Authentication authentication) {
        return orderService.getOrdersHistory(userService.findByEmail(
                authentication.getName()).orElseThrow(
                        () -> new RuntimeException("Couldn't get order history for: "
                                + authentication.getName())))
                .stream().map(orderResponseDtoMapper::mapToDto).toList();
    }
}
