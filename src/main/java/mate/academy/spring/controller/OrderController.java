...
public class OrderController {
    ...
    public OrderResponseDto completeOrder(@RequestBody Authentication authentication) {
        String email = authentication.getName();
        User user = userService.findByEmail(email)
                .orElseThrow(
                        () -> new DataProcessingException("Can't find user by email: " + email));
        ShoppingCart cart =
                shoppingCartService.getByUser(userService.get(user.getId()));
        return responseDtoMapper.mapToDto(orderService.completeOrder(cart));
    }

    @GetMapping
    public List<OrderResponseDto> getOrderHistory(@RequestBody Authentication authentication) {
        return orderService
                .getOrdersHistory(userService.findByEmail(authentication.getName()).get())
                .stream()
