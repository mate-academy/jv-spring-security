...
public class OrderController {
    ...
    public OrderResponseDto completeOrder(Authentication authen) {
        ShoppingCart cart = shoppingCartService
                .getByUser(userService.findByEmail(authen.getName()).get());
        return orderResponseDtoMapper.mapToDto(orderService.completeOrder(cart));
    }

    @GetMapping
    public List<OrderResponseDto> getOrderHistory(Authentication authen) {
        return orderService.getOrdersHistory(userService
                        .findByEmail(authen.getName()).get())
                .stream()
