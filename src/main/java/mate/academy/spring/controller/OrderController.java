...
public class OrderController {
    ...
    public OrderResponseDto completeOrder(Authentication authentication) {
        ShoppingCart cart = shoppingCartService
                .getByUser(getUserByPrincipal(authentication));
        return orderResponseDtoMapper.mapToDto(orderService.completeOrder(cart));
    }

    @GetMapping
    public List<OrderResponseDto> getOrderHistory(Authentication authentication) {
        return orderService.getOrdersHistory(getUserByPrincipal(authentication))
                .stream()
    ...
    private User getUserByPrincipal(Authentication authentication) {
        ...
        return userService.findByEmail(login).orElseThrow(() ->
                new EntityNotFoundException("Can't find user with login: " + login));
    }
}
